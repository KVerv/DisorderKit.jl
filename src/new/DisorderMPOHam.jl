...
#     Rs::Vector{<:AbstractTensorMap{T, S, 2, 1}}
#     Ls::Vector{<:AbstractTensorMap{T, S, 1, 2}}
...
#     length(As) == length(Rs) == length(Ls) == length(Ds) || error("All input vectors must have the same length")
#     return DisorderMPOHam(As, Rs, Ls, Ds)
# end

# function random_transverse_field_ising(Js::Vector{Float64}, hs::Vector{Float64})
#     A = zeros(ComplexF64, ℂ^1⊗ℂ^2, ℂ^2⊗ℂ^1)
#     R = TensorMap([(1. +0im) 0.; 0. -1.], ℂ^1⊗ℂ^2, ℂ^2)
#     L =  TensorMap([(1. +0im) 0.; 0. -1.], ℂ^2, ℂ^2⊗ℂ^1)
