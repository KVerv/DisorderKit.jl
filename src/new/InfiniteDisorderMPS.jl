...
function average_entanglement_entropy(ρ::InfiniteDisorderMPS)
    ...
    # @tensor T[-1 -2 -3 -4; -5 -6 -7 -8] := OR[-2 1; -1 3] * conj(OR[-3 1; -4 4]) * OR[3 5; -5 -6] * conj(OR[4 5; -8 -7])
    # @tensor T[-1 -2 -3 -4; -5 -6 -7 -8] := ρ[1][-2 1 2; -1 3] * conj(ρ[1][-3 1 2; -4 4]) * ρ[1][3 5 6; -5 -6] * conj(ρ[1][4 5 6; -8 -7])
    # @tensor T[-1 -2; -3 -4] := ρ[1][-1 1 2; 3 -3] * conj(ρ[1][-2 1 2; 3 -4])
    # # λs = svd_vals(T)
    # λs = eig_vals(T)
    # @show λs
...
function partition_function(ρ::InfiniteDisorderMPS)
    A = ρ[1]
    iso = isomorphism(ComplexF64, fuse(space(A, 1) ⊗ space(A, 1)'), space(A, 1) ⊗ space(A, 1)')
    @tensor Z[-1 -2; -3 -4] := iso[-1; 1 4] * A[1 2 3; -3 5] * conj(A[4 2 3; -2 6]) * conj(iso[-4; 5 6])
    return Z
end