...
function InfiniteDisorderDensityMatrix(ps::Vector{Float64}, pspace::ElementarySpace, tspace::ElementarySpace, vspace::ElementarySpace; T=ComplexF64, σ::Float64=1e-1)
    D_dis = length(ps)
    oppbar = randn(T, vspace ⊗ pspace, tspace ⊗ vspace)
    opp = [oppbar + σ*randn(T, vspace ⊗ pspace, tspace ⊗ vspace) for i in 1:D_dis]
    return gauge!(InfiniteDisorderDensityMatrix(opp, ps))
end
...
function Base.:*(ρs::InfiniteDisorderDensityMatrix, O::DisorderMPO)
    ...
    return gauge!(InfiniteDisorderDensityMatrix(opp, ρs.ps))
end

# Multiply DensityMatrix with DensityMatrix
function Base.:*(ρs::InfiniteDisorderDensityMatrix, Os::InfiniteDisorderDensityMatrix)
    (length(ρs.opp) == length(Os.opp)) || throw(ArgumentError("O should have the same amount of disorder sectors as ρ"))

    opp = Vector{typeof(ρs.opp[1])}(undef, length(ρs.opp))
    for (p, ρ) in  enumerate(ρs)
        iso = isomorphism(ComplexF64, fuse(space(ρ, 1), space(Os[p],1)), space(ρ, 1) ⊗ space(Os[p],1))
        @show space(ρ), space(Os[p])
        @tensor ρnew[-1 -2; -3 -4] := iso[-1; 1 2]*ρ[1 3;-3 4] *Os[p][2 -2;3 5] * conj(iso[-4; 4 5])
        opp[p] = ρnew
    end
...
function environments(ρs::InfiniteDisorderDensityMatrix)
...
function right_second_moment_transfer_matrix(ρs::InfiniteDisorderDensityMatrix)
    function ftransfer(vr)
        v = zeros(ComplexF64,space(ρs[1],1)'⊗space(ρs[1],1),space(ρs[1],1)⊗space(ρs[1],1)')
        for (p, A) in enumerate(ρs)
            @tensor vp[-1 -2; -3 -4] := A[-2 4; 3 2] * conj(A[-1 4;3 1]) * A[-4 8; 7 6] * conj(A[-3 8;7 5]) * vr[1 2; 5 6]
            v += ρs.ps[p]*vp
        end
        return v
    end
...
function left_second_moment_transfer_matrix(ρs::InfiniteDisorderDensityMatrix)
    function ftransfer(vl)
        v = zeros(ComplexF64,space(ρs[1],1)⊗space(ρs[1],1)',space(ρs[1],1)'⊗space(ρs[1],1))
        for (p, A) in enumerate(ρs)
            @tensor vp[-1 -2; -3 -4] := A[2 4; 3 -4] * conj(A[1 4;3 -3]) * A[6 8; 7 -2] * conj(A[5 8;7 -1]) * vl[5 6; 1 2]
            v += ρs.ps[p]*vp
        end
        return v
    end
...
function gauge!(ρs::InfiniteDisorderDensityMatrix)
    λ, _ = left_environment(ρs)
    return rescale(ρs, 1/sqrt.(λ)*ones(length(ρs)))
end

function norm_moments(ρs::InfiniteDisorderDensityMatrix)
    λl, vl, vr = DisorderKit.environments(ρs)
    N1 = λl
    ftransfer = right_second_moment_transfer_matrix(ρs)
    vr0 = rand(ComplexF64, space(ρs[1],1)'⊗space(ρs[1],1), space(ρs[1],1)⊗space(ρs[1],1)')
    λ2r, vr2 = eigsolve(x->ftransfer(x), vr0, 1, :LM)
    N2 = λ2r[1]
...
function correlationfunc(ρs::InfiniteDisorderDensityMatrix, O::AbstractBondTensor, r::Int)
    Os = [O for i in 1:length(ρs.opp)]

    return correlationfunc(ρs, Os, r)
end

function correlationfunc(ρs::InfiniteDisorderDensityMatrix, Os::Vector{<:AbstractBondTensor}, r::Int)
    λl, vl, vr = DisorderKit.environments(ρs)
    ft = left_transfer_matrix(ρs)

    CL = zeros(ComplexF64, space(vl))
    CR = zeros(ComplexF64, space(vr))
    for (p, ρ) in enumerate(ρs)
        @tensor CLp[-1; -2] := vl[1; 2] * ρ[2 4; 3 -2] * Os[p][5;4] * conj(ρ[1 5; 3 -1])
        CL += ρs.ps[p]*CLp

        @tensor CRp[-1; -2] := vr[1; 2] * ρ[-1 4; 3 1] * Os[p][5;4] * conj(ρ[-2 5; 3 2])
        CR += ρs.ps[p]*CRp
    end
    Cs = zeros(ComplexF64, 1:r)
    for i in 1:r
        if i>1
            CL = ft(CL)
        end
...
# measure average correlation length (assuming proper gauge)
...
function energy_density(ρs::InfiniteDisorderDensityMatrix, Hs::DisorderMPOHam)
    _, l, r = DisorderKit.environments(ρs)
    E = 0
    for (p, ρ) in enumerate(ρs)
        D = Hs.Ds[p]
        C = Hs.Cs[p]
...
function normalize_density_matrix(ρ::InfiniteDisorderDensityMatrix)
    ρ = DisorderKit.gauge!(ρ)
    ρ_product = DisorderKit.renormalize(ρ; χ=1, N=1)
    ρ_product = DisorderKit.gauge!(ρ_product)
    # es = entanglement_spectrum_norm(ρ_product)
    # ϵ = sum(es[1:end-1])
    # @show es
